        ...
        <div className="text-break">
            {expanded ? (
                ...
                <>
                    ...
                    <button className="btn btn-link p-0 ms-1 text-decoration-none" onClick={() => setExpanded(true)}>
        ...
        case StatementTypes.RevealAttribute:
            return (
                <div className="p-2 border-start border-info border-3 ps-3 mb-2">
                    <div>
                        <strong>Type:</strong> <span className="badge bg-info">Reveal Attribute</span>
                    </div>
                    <div>
                        <strong>Attribute:</strong> {statement.attributeTag}
                    </div>
                    {statement.revealed && (
                        <div className="mt-2">
        ...
        case StatementTypes.AttributeInRange:
            return (
                <div className="p-2 border-start border-warning border-3 ps-3 mb-2">
                    <div>
                        <strong>Type:</strong> <span className="badge bg-warning text-dark">Attribute In Range</span>
                    </div>
                    <div>
                        <strong>Attribute:</strong> {statement.attributeTag}
                    </div>
                    <div>
                        <strong>Lower Bound:</strong> {formatValue(statement.lower)}
                    </div>
                    <div>
                        <strong>Upper Bound:</strong> {formatValue(statement.upper)}
                    </div>
                </div>
            );
        case StatementTypes.AttributeInSet:
            return (
                <div className="p-2 border-start border-success border-3 ps-3 mb-2">
                    <div>
                        <strong>Type:</strong> <span className="badge bg-success">Attribute In Set</span>
                    </div>
                    <div>
                        <strong>Attribute:</strong> {statement.attributeTag}
                    </div>
                    <div>
                        <strong>Set:</strong>{' '}
                        {Array.isArray(statement.set)
                            ? statement.set.map(formatValue).join(', ')
                            : formatValue(statement.set)}
                    </div>
                </div>
            );
        case StatementTypes.AttributeNotInSet:
            return (
                <div className="p-2 border-start border-danger border-3 ps-3 mb-2">
                    <div>
                        <strong>Type:</strong> <span className="badge bg-danger">Attribute Not In Set</span>
                    </div>
                    <div>
                        <strong>Attribute:</strong> {statement.attributeTag}
                    </div>
        ...
        default:
            return (
                <div className="p-2 border-start border-secondary border-3 ps-3 mb-2">
                    <div>
                        <strong>Type:</strong> <span className="badge bg-secondary">{statement.type}</span>
                    </div>
                    <pre className="my-2 bg-light p-2 rounded small">{JSON.stringify(statement, null, 2)}</pre>
                </div>
            );
    }
};
...
// Raw data view
const renderRawView = (proof: Proof, proofSize: number, onClose: () => void, onSwitchView?: () => void) => (
    <div
        ...
        >
            ...
            <div className="d-flex justify-content-between mb-3">
                <span className="text-muted">
                    Size: {formatSize(proofSize)} ({proof.toString().length} characters)
                </span>
                {onSwitchView && (
                    <button className="btn btn-sm btn-outline-primary" onClick={onSwitchView}>
...
// Structured view
const renderStructuredView = (proof: Proof, proofSize: number, onClose: () => void, onSwitchView?: () => void) => {
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <div className="ps-3 mt-1">
                                                {Array.isArray(credentialSubject.proof.proofValue) ? (
                                                    credentialSubject.proof.proofValue.map((item: any, i: number) =>
                                                        renderProofItem(item, i)
                                                    )
                                                ) : (
                                                    <TruncatedText
                                                        text={
                                                            typeof credentialSubject.proof.proofValue === 'string'
                                                                ? credentialSubject.proof.proofValue
        ...
        >
            ...
            >
                <div className="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div>
                        {onSwitchView && (
                            <button className="btn btn-sm btn-outline-primary me-2" onClick={onSwitchView}>
                                View Raw Data
                            </button>
                        )}
                        <button type="button" className="btn-close" onClick={onClose}></button>
                ...
                {proof.value.verifiableCredential && proof.value.verifiableCredential.length > 0 && (
                    <div className="mb-3">
                        ...
                        {proof.value.verifiableCredential.map((credential: any, index: number) => (
                            <div key={index}>{renderCredential(credential)}</div>
                        ))}
                    </div>
                )}

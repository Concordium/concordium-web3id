...
function Layout({ platform, children, logo }: Props) {
  ...
  const [txState, setTxState] = useState<
    Pick<AppState, 'transaction' | 'transactionStatus'>
  >({
  ...
  return (
    <appState.Provider value={appStateValue}>
      <main className="layout">
        ...
        <h4 className="mb-4">
          Create your Web3 ID credential for {platform} by logging in. This
          process includes the steps:
        </h4>
        <ol className="text-start mb-5">
          <li>Log in with {platform}</li>
          <li>Accept request to add credential to your wallet</li>
          <li>
            Finalization of credential registration on Concordium{' '}
            {config.network}
          </li>
        </ol>
        {children}
        ...
        <div className="d-flex align-items-center flex-wrap">
          <span className="me-auto text-body text-opacity-25">
            v{manifest.version}
          </span>

...
export async function requestCredential(
    ...
    const credential = {
        ...
        type: ['VerifiableCredential', 'ConcordiumVerifiableCredential', 'SoMeCredential'],
        issuer: CONTRACT_DID,
    ...
    await api.addWeb3IdCredential(credential, metadataUrl, async (id) => {
        ...
        if (!response.ok) throw new Error('Error getting credential: ' + (await response.text()));

        const { txHash: hash, credential } = (await response.json()) as IssuerResponse;
        txHash = hash;
